...
The `Data Access` API requires... (haven't written anything to explore getting the data)
...
search_type <- "&type=dataset&metadata_fields=Distribution%20Date:*" # limit to datasets
```
...
We don't need all this data. We should be able to do some evaluation with the `name`, `description`, `fileCount`, `producers`, `published_at` and, for later retrieval, `url`. Let's look at these quickly, noting importantly, that `producers` is another list, that we'll need to wrangle some how.

```{r, vars-of-interest}
vars_of_interest <- c("name", "description", "fileCount", "producers", "published_at", "url")
...
search_datasets <- function(query, per_page) {
  ...
  while(length(handle) < total_count) { # while the number of retrieved records is < the total count
    ...
    resp_body_pubdate <- resp_body$data$items$published_at
