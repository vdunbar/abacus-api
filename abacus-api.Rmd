...
After loading the base url, we can add on the specific API we intend to search. We'll store these for later use as well.
...
```{r, query}
search_query <- "*" # retrieve everything with a wildcard
...
What wer're most interested in here is the `body`. However, the `body` is sent to us in raw bytes. This is geneally the case in any API response.
...
We don't need all this data. We should be able to do some evaluation with the `name`, `description`, `fileCount`, `producers`, and, for later retrieval, `url`. Let's look at these quickly, noting importantly, that `producers` is another list, that we'll need to wrangle some how.

```{r, vars-of-interest}
vars_of_interest <- c("name", "description", "fileCount", "producers", "url")
...
* APIs will have a limit on the number of requests you can make in any given time span.
...
```{r, search-function-call, cache = TRUE}
all_datasets <- search_datasets("*", 500)
```
...
We can make this a bit easier for ourselves by converting this into a dataframe or tibble. There are several ways to approach this. Here, I'll treat datasets with multiple producers as a different record authority than if one of these co-producers produced a standalone dataset. To do this, we'll first collapse the `producers` nested lists, and then flatten the result into a single list.

```{r, all-datasets-df-1}
all_datasets$producers <- all_datasets$producers |>
  lapply(function(x) paste(x, collapse = ", ")) |>
  unlist()

head(all_datasets$producers, n = 20)
```

We can easily convert this to a tibble.

```{r, all-datasets-df-2}
all_datasets <- tidyr::as_tibble(all_datasets)
head(all_datasets)
```

## Explore

Let's first get a sense of who's producing datasets in Abacus and how many datasets they have.

```{r}
producer_table <- table(all_datasets$producers) |>
  as.data.frame() |>
  dplyr::arrange(desc(Freq))

knitr::kable(producer_table, col.names = c("Producers", "Count")) |>
  kableExtra::kable_styling() |>
  kableExtra::scroll_box(height = "400px")
```

Find those with 'labour' in them

```{r, labour-search}
labour_records <- all_datasets[grep("labour", all_datasets$description, ignore.case = TRUE), ]
```